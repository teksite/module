...
trait ModuleMigrationTrait
{
    ...
    public function moduleMigrationFiles(?string $moduleName = null,): array
    {
        ...
        $migrationsPath = module_path($moduleName, config('modules.module.migration_path', 'database/migrations'));

        if (!File::isDirectory($migrationsPath)) return [];
    ...
    public function seedingModules(): void
    {
        ...
        foreach ($modules as $module) {
            $mainSeeder = moduleNamespace($module)."\\Database\\Seeders\\{$module}DatabaseSeeder";

            if (class_exists($mainSeeder)) {
                $this->runAndCalculate(function () use ($mainSeeder) {
                    app($mainSeeder)->__invoke();
                }, "seeding $module");
            }
        }
    }
